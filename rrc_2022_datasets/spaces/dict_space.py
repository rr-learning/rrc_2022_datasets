...
class DictSpace(Space):
    ...
    def flatten_value(self, value, keys=None):
        ...
        flat_sub_values = [
            self._space_dict[key].flatten_value(value[key]) for key in keys
        ]
    ...
    def unflatten_value(self, flat_value, keys=None):
        ...
        value = {
            key: self._space_dict[key].unflatten_value(
                flat_value[indices[i] : indices[i + 1]]
            )
    ...
    def get_gym_space(self):
        dict_of_gym_spaces = {
            key: space.get_gym_space() for key, space in self._space_dict.items()
        }
        return gym_spaces.Dict(dict_of_gym_spaces)

    def __eq__(self, other):
        if not (isinstance(other, DictSpace) and self.keys() == other.keys()):
            return False
    ...
    def get_from_gym_space(gym_space):
        ...
        return DictSpace(
            {key: space_from_gym_space(gs) for key, gs in gym_space.spaces.items()}
        )
    ...
    def get_subspace(self, keys):
        return DictSpace(
            {key: space for key, space in self._space_dict.items() if key in keys}
        )
