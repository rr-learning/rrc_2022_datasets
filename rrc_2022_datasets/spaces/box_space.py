...
class BoxSpace(Space):
    def __init__(self, low, high, dtype=np.float32):
        ...
        self.n = np.prod(np.shape(self.low))
    ...
    def get_flat_space(self, indices=None):
        low = np.reshape(self.low, (self.n,))
        high = np.reshape(self.high, (self.n,))
        if indices is not None:
            low = low[indices]
            high = high[indices]
        space = BoxSpace(low=low, high=high, dtype=self.dtype)
        return space
        ...
        for a in [False, True]:
            for b in [False, True]:
                ...
                if b == True and a == True:
                    res[applies] = (
                        self._difference[applies]
                        * np.random.random(size=np.sum(applies))
                        + self.low[applies]
                    )
                # bounded from below
                if b == True and a == False:
                    res[applies] = (
                        np.random.exponential(size=np.sum(applies)) + self.low[applies]
                    )
                # bounded from above
                if b == False and a == True:
                    res[applies] = (
                        -np.random.exponential(size=np.sum(applies))
                        + self.high[applies]
                    )
                # unbounded
    ...
    def __eq__(self, other):
        if not isinstance(other, BoxSpace):
            return False
        return (
            np.all(np.equal(self.low, other.low))
    ...
    def get_from_gym_space(gym_space):
        return BoxSpace(low=gym_space.low, high=gym_space.high, dtype=gym_space.dtype)

    def __str__(self):
        return "BoxSpace(low = {}; high = {}; dtype = {})".format(
            self.low, self.high, self.dtype
        )
